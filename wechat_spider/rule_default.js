...
module.exports = {
    ...
    replaceServerResDataAsync: function(req,res,serverResData,callback){
        ...
        if(/mp\/getmasssendmsg/i.test(req.url)){//当链接地址为公众号历史消息页面时(第一种页面形式)
            if(serverResData.toString() !== ""){
                try {//防止报错退出程序
                    ...
                    HttpPost(ret[1],req.url,"/process_history_list");//这个函数是后文定义的，将匹配到的历史消息json发送到自己的服务器
                    var http = require('http');
                    http.get('http://127.0.0.1:8080/get_history_list', function(res) {//这个地址是自己服务器上的一个程序，目的是为了获取到下一个链接地址，将地址放在一个js脚本中，将页面自动跳转到下一页。后文将介绍getWxHis.php的原理。
                     ...
                     try {
                        ...
                        HttpPost(json.general_msg_list,req.url,"/process_history_list");//这个函数和上面的一样是后文定义的，将第二页历史消息的json发送到自己的服务器
                        }
                     }catch(e){
                       console.log(e);//错误捕捉
                     }
            ...
            }else{//如果为空也要返回response，不然的话网页无法重定向
				callback(serverResData);
			}
        ...
        // }else if(/mp\/getappmsgext/i.test(req.url)){//当链接地址为公众号文章阅读量和点赞量时
            // try {
                // HttpPost(serverResData,req.url,"/process_read_and_like_count");//函数是后文定义的，功能是将文章阅读量点赞量的json发送到服务器
            // }catch(e){
            ...
            try {
				HttpPost(serverResData.toString(),req.url,"/process_article");
                var http = require('http');
                http.get('http://localhost:8080/get_next_article', function(res) {//这个地址是自己服务器上的另一个程序，目的是为了获取到下一个链接地址，将地址放在一个js脚本中，将页面自动跳转到下一页。后文将介绍getWxPost.php的原理。
                    res.on('data', function(chunk){
