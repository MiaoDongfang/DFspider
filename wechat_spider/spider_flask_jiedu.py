...
# COVER_IMAGE_DIRECTORY = '/mnt/static_files/LKXY_IMG'
if platform.system() == "Windows":
    IMAGE_DIRECTORY = 'wx_images/'
    COVER_IMAGE_DIRECTORY = 'cover_images/'
elif platform.system() == "Linux":
    IMAGE_DIRECTORY = '/mnt/static_file/weixin_images/'
    COVER_IMAGE_DIRECTORY = '/mnt/static_file/LKXY_IMG/'
else:
    IMAGE_DIRECTORY = '/mnt/static_file/weixin_images/'
    COVER_IMAGE_DIRECTORY = '/mnt/static_file/LKXY_IMG/'
...
def process_article():
            ...
            if src:
                ...
                if resp.status_code == requests.codes.ok:
                    ...
                    with open("%s%s" % (IMAGE_DIRECTORY, image_name), "wb") as fp:
                        ...
                        logger.info("图片下载成功, 保存路径%s%s" % (IMAGE_DIRECTORY, image_name))
    ...
    if len(articles) == 1:
        ...
        if cover:
            headers = {"Host": "mmbiz.qpic.cn", "Upgrade-Insecure-Requests": '1',
                       "User-Agent": random.choice(CONFIG_USERAGENT_ALL)}
            resp = requests.get(cover, headers=headers)
            if resp.status_code == requests.codes.ok:
                wx_fmt = get_url_query(cover, "wx_fmt")
                if wx_fmt is None:
                    wx_fmt = 'png'
                cover_name = db.next_id() + '.' + wx_fmt
                logger.info("正在下载图片 %s" % cover)
                with open("%s%s" % (COVER_IMAGE_DIRECTORY, cover_name), "wb") as fp:
                    fp.write(resp.content)
                    logger.info("图片下载成功, 保存路径%s%s" % (COVER_IMAGE_DIRECTORY, cover_name))
            else:
                logger.error("图片下载失败%s" % cover)
                cover_name = None
        with db.transaction():
            ...
            if content is None or content == "" or article["title"] == "" or article["title"] is None:
                ...
                article_lk = {
                    "readhotName": article["title"],
                    "readhotContent": content,
                    "readhotTime": article["datetime"],
                    "readhotSource": author,
                    "readhotAuthor": author,
                    "readhotHeat": 0,
                    "readhotImgurl": cover_name,
                    "readhotUrl": article["content_url"],
                    "readhotType": 1,
                    "readhotComment": 0,
                }
                db.insert("readandhot", **article_lk)
                logger.info("文章保存成功")
    return "success"
